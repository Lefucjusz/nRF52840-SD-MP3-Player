...
// #define PLAYER_REQUEST_TIMEOUT_MS 500
...
static int decode_and_push_stream(void)
{
    ...
    err = k_mem_slab_alloc(&ctx.i2s_mem_slab, &block, K_FOREVER);
    if (err) {
        return err;
    }
...
void player_task(void)
{
    ...
    err = k_mem_slab_init(&ctx.i2s_mem_slab, ctx.i2c_mem_slab_buf, PLAYER_I2S_BLOCK_SIZE, PLAYER_I2S_BUFFER_BLOCKS);
    if (err) {
        ...
        do {
            ...
            while (1) {
                ...
                if (ctx.state == PLAYER_PLAYING) {
                    ...
                    k_msleep(100); // Give other tasks some time
                }
            }
        } while (0);
