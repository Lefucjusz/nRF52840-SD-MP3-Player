...
static const struct device *display = DEVICE_DT_GET(DT_ALIAS(oled));
...
void ssd1306_UpdateScreen(void) {
    const struct display_buffer_descriptor buf_desc = {
        ...
        .buf_size = sizeof(SSD1306_Buffer),
        .pitch = SSD1306_WIDTH
    };

    display_write(display, 0, 0, &buf_desc, SSD1306_Buffer);
}
...
char ssd1306_WriteChar(char ch, SSD1306_Font_t Font, SSD1306_COLOR color) {
    ...
    for(i = 0; i < Font.height; i++) {
        b = Font.data[(ch_to_show - 32) * Font.height + i];
    ...
    SSD1306.CurrentX += Font.char_width ? Font.char_width[ch_to_show - 32] : Font.width;
